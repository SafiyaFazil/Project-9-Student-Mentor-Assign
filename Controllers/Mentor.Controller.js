...
export const createMentor = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Mentor created successfully", data: mentorDetails });
  } catch (error) {
...
export const getMentorList = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Mentor list fetched successfully", data: mentors });
  } catch (error) {
...
export const previousMentor = async (req, res) => {
  try {
    ...
    if (student) {
      ...
      return res
        .status(200)
        .json({ message: "Previous mentor found", data: preMentor });
    }
...
export const assignStudentToMentor = async (req, res) => {
  ...
  try {
    ...
    if (!mentor)
      res
        .status(404)
        .json({ message: "mentor not found please create mentor" });
    console.log(mentor);
    ...
    if (!Student)
      res
        .status(404)
        .json({ message: "Student not found please create Student" });

    if (Student.Mentor == mentor.MentorName)
      return res
        .status(403)
        .json({
          message: "Mentor already assigned please enter alternate mentor name",
        });
    ...
    return res
      ...
      .json({
        message: "Student assigned to mentor successfully",
        data: PrevNewMentorforStudent,
      });
  } catch (error) {
    console.error("Error assigning student to mentor:", error);
    res
      .status(500)
      .json({ message: "Error in assigning student to mentor", error });
  }
};
...
export const MultiStudForOneMen = async (req, res) => {
  try {
    ...
    if (UpdatedStudents.matchedCount !== 0) {
      ...
      return res
        .status(404)
        .json({ message: "No Matching found for the students" });
    }
  } catch (error) {
    console.error("Error assigning student to mentor:", error);
    res
      .status(500)
      .json({
        message: "Error in assigning multi student to one mentor",
        error,
      });
  }
};
...
export const getStudentsForMentor = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error in fetching students for mentor", error });
  }
};
